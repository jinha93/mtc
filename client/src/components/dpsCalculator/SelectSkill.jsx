...
export default function SelectSkill(props) {
    ...
    }, [selectSkillIndex, skillData, statData, normalOrBoss]) 
    ...
    const calc = () => {
        ...
        // 대미지 = 공격력 * (1+모든대미지) * (1+스킬대미지) * (1+거리별대미지) * 스킬계수 * 스킬타수 * 보스공격대미지 * (1+속성대미지+버프대미지)
        const damage = ((attack * (1+allDamageRate) * (1+skillDamageRate) * (1+distanceDamage)) * totalCoefficient * hitCnt * bossAttackDamage * (1+attributeDamage+buffDamage)).toFixed(1);
        // 대미지(치명타) = 대미지 * 치명타대미지
    ...
    // useEffect(() => {
    //     if(props.skillsData[selectSkillIndex].id === 'default') return;
    //     // calc();
    //     // eslint-disable-next-line react-hooks/exhaustive-deps
    // }, [selectSkillIndex, props.skillsData, props.stats, props.normalOrBoss])    

    // const calc = () => {
    //     // 스탯
    //     const attack = props.stats.find(v => v.id === 'attack').value
    //     const attackSpeed = props.stats.find(v => v.id === 'attackSpeed').value
    ...
    //     const meleeDamageRate = props.stats.find(v => v.id === 'meleeDamageRate').value/100
    //     const rangedDamageRate = props.stats.find(v => v.id === 'rangedDamageRate').value/100

    //     const fireDamageRate = props.stats.find(v => v.id === 'fireDamageRate').value/100
    //     const waterDamageRate = props.stats.find(v => v.id === 'waterDamageRate').value/100
    //     const windDamageRate = props.stats.find(v => v.id === 'windDamageRate').value/100
    //     const groundDamageRate = props.stats.find(v => v.id === 'groundDamageRate').value/100
    //     const electricDamageRate = props.stats.find(v => v.id === 'electricDamageRate').value/100
    //     const iceDamageRate = props.stats.find(v => v.id === 'iceDamageRate').value/100
    //     const plantDamageRate = props.stats.find(v => v.id === 'plantDamageRate').value/100
    //     const poisonDamageRate = props.stats.find(v => v.id === 'poisonDamageRate').value/100
    //     const lightDamageRate = props.stats.find(v => v.id === 'lightDamageRate').value/100
    //     const darknessDamageRate = props.stats.find(v => v.id === 'darknessDamageRate').value/100
    ...
    //     // 버프 속성 대미지
    //     // if(type === '버프') props.setBuffArrIndex(props.index, props.skillsData[selectSkillIndex]);
    //     if(type === '버프'){
    //         props.buffArr[props.index] = props.skillsData[selectSkillIndex]
    //         const arr = [...props.buffArr];
    //         props.setBuffArr(arr);
    //     }
    //     let buffDamage = 0;
    //     for(let buff of props.buffArr){
    //         buffDamage = attribute === buff.attribute ? calcCoefficient(buff)/100 : 0;
    //     }
    //     console.log(props.skillsData[selectSkillIndex])
    //     console.log(buffDamage);
    //     // console.log(totalCoefficient, buffDamage);
    //     // 속성 대미지
    //     let  attributeDamage = 0;
    //     switch (attribute) {
    //         case '불':
    ...
    //     // 대미지 = 공격력 * (1+모든대미지) * (1+스킬대미지) * (1+거리별대미지) * 스킬계수 * 스킬타수 * 보스공격대미지 * (1+속성대미지) * (1+버프대미지)
    //     const damage = ((attack * (1+allDamageRate) * (1+skillDamageRate) * (1+distanceDamage)) * totalCoefficient * hitCnt * bossAttackDamage * (1+attributeDamage)).toFixed(1);
    //     // 대미지(치명타) = 대미지 * 치명타대미지
    //     const damageCritical = (damage * criticalDamage).toFixed(1);
    //     // DPS = (대미지 * 치명타확률 * 치명타 대미지 + 대미지 * (1-치명타확률)) / (쿨타임)
    //     const dps = ((damage*criticalRate*criticalDamage+damage*(1-criticalRate))/cooldown).toFixed(1);
    ...
    // useEffect(() => {
    //     setDamage(_damage * (1.3));
    // }, [props.buffArr, props.skillsData])
    ...
    return (
        <div className="border-2 border-gray-900 rounded-lg w-full h-full p-3">
            <div className="md:flex">
                ...
                <select className="w-full border rounded-lg border-gray-900 text-gray-700 text-sm my-1 py-1 md:mx-1 md:my-3 md:py-0" onChange={onChangeSkill} value={selectSkillIndex}>
